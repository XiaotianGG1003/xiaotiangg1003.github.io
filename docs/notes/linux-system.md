...
:::tip
Linux 中推荐`sigaction()`信号处理函数，不仅可以注册处理函数，还支持屏蔽信号，自动重启、其他扩展等。
:::

#### 进程级阻塞
* 信号产生时，若进程未阻塞该信号，内核直接递送；若已阻塞，信号进入未决状态（不可靠信号去重，可靠信号排队），待阻塞解除后才递送。
* 当信号处理函数执行时，若再次产生**同类型信号**(仅针对不可靠信号)，内核会维护一个**未决队列**，仅保留一个，后续相同类型信号会被丢弃，函数执行完后，内核会递送信号。可靠信号按顺序排队。
* 当信号处理函数执行时，若再次产生**不同类型信号(未被阻塞)**，内核会暂停处理当前信号，执行新信号的处理函数，执行完后继续处理当前信号，确保进程可以被终止
* 阻塞时的信号处理：低速系统调用(read,wait)时，信号产生会中断调用，处理信号完，然后重启系统调用。非低速调用，信号递送时会返回错误，然后处理信号，然后根据错误返回值，决定是否重试系统调用。
```cpp
int main() 
{
    sigset_t set;
    ...
    cout << "SIGINT is blocked now." << endl;
    sleep(5);
    ret = sigprocmask(SIG_UNBLOCK, &set, NULL);
    cout << "SIGINT is unblocked now." << endl;
    return 0;
}
```
